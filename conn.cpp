...
RET_CODE conn::read_clt()
{
    ...
    while( true )
    {
        ...
        if( m_clt_read_idx >= BUF_SIZE )//读超过缓冲区，打印失败日志
        {
        ...
        if ( bytes_read == -1 )
        {
            if( errno == EAGAIN || errno == EWOULDBLOCK )//无数据可读了
            {
                break;//结束本次read_clt操作,下次重新读
            }
            log( LOG_ERR, __FILE__, __LINE__, "read client socket failed, %s", strerror( errno ) );
            return IOERR;
        }
...
RET_CODE conn::read_srv()
{
    ...
    while( true )
    {
        ...
        if ( bytes_read == -1 )
        {
            ...
            log( LOG_ERR, __FILE__, __LINE__, "read server socket failed, %s", strerror( errno ) );
            return IOERR;
        }
...
RET_CODE conn::write_clt()
{
    ...
    while( true )
    {
        ...
        if ( bytes_write == -1 )
        {
            if( errno == EAGAIN || errno == EWOULDBLOCK )
            {
                return TRY_AGAIN;//为什么不跳出循环，返回OK???
            }
